...
class Sherloock:
    ...
    def _handle_parallel_solve(self, problem_str: str, constraint_str: Optional[str] = None):
        ...
        with ThreadPoolExecutor(max_workers=self.max_fibers) as ex:
            ...
            for f in as_completed(futures):
                ...
                if r:
                    results.append(r)
        return f"[MÚSCULO LÓGICO] (Demo para '{problem_str}') {len(results)} soluciones: {results}"
    ...
    def _forecast(self, data: List[float], limit: Optional[float] = None):
        ...
        if limit is not None:
            ...
            return f"[Z3+PuLP] Próximo valor (límite {limit:.2f}): {next_val_opt:.2f}"
        return f"[FORECAST PuLP] Próximo valor optimizado: {next_val_opt:.2f}"
